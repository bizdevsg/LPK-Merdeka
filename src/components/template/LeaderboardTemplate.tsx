...
import { LineHeading } from "../molecules"; // Import LineHeading if available or use standard h2
          ...
          return (
            <div key={user.id} className={`flex flex-col items-center w-full md:w-1/3 group ${orderClass}`}>
              ...
              <div className="text-center mb-4">
                <h3 className={`font-bold text-gray-900 ${rank === 1 ? 'text-xl' : 'text-lg'}`}>{user.title}</h3>
                <p className="text-sm text-gray-500">{user.description}</p>
                <p className={`font-bold mt-1 ${rank === 1 ? 'text-yellow-600 text-lg' : 'text-gray-600'}`}>{user.score} XP</p>
              </div>
